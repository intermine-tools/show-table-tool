...
window.onload = function() {
  var chan, sessionRequest, withSession;
  chan = Channel.build({
  ...
  chan.bind('change-state', function(trans, data) {
    return console.log('CHANGED STATE', data);
  });
  chan.bind('has-list', function(trans, data) {
    return console.log('HAS LIST', data);
  });
  chan.bind('next-step', function(trans, data) {
    return console.log('NEXT STEP', data);
  });
  chan.bind('changed-list', function(trans, data) {
    return console.log('CHANGED LIST', data);
  });
  chan.bind('has', function(trans, params) {
    ...
    if (params.what === 'item') {
      obj = params.data;
      return alert("Chose " + (obj.classes.join(', ')) + " " + obj.id);
    } else if (params.what === 'query') {
      query = JSON.stringify(params.data, null, 2);
      return document.getElementById('stdout').innerHTML = query;
    }
  });
  withSession = function(req, e) {
    ...
    return chan.call({
      ...
      success: function() {
        return console.log("Table initialised");
      },
      error: function(e) {
        return console.error("Could not initialise table", e);
      }
    });
  };
  ...
  return sessionRequest.send();
};



},{"jschannel":2}],2:[function(require,module,exports){
/*
